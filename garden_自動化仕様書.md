...
8. wardlichen: レーン2で cronerice(既設があれば流用、なければ@(0,4),(2,4),(4,4)) / whiteMildew @ (1,4),(3,4),(5,4)(非汚染・標準形。3本目の白カビで(5,3),(5,5)も抽選マスになり計12個 — 白カビのゴミ湧き・植え替えコストはほぼゼロなので枚数を惜しまない)→ 解禁
...
3. **格子植付**: queenbeet を「x,yがともに奇数の9マス」**以外の27マス**に植付(コスト60分CpS/本のためバフガード厳守・分割植付可)。9マスのうち(5,5)は手順2の扱い、残り8マスが変異穴。開始条件は「late3種以外の全種が確保済み(解禁or芽)かつqueenbeet解禁」で、everdaisy等の芽が残っていても建設を始める(芽のマスは収穫まで避け、空き次第埋める)。**縁の刈込**: duketaterとshriekbulbの両方を確保したら(JQB湧き前でも)、x=5またはy=5の縁11マス(⑤穴4個+それにしか寄与しないqueenbeet6本+(5,5))を**bakerWheatに転用**する。JQBは隣接8マスの成熟queenbeetを要求するため縁の穴(隣接5マス)では原理的に湧けず、生きた⑧穴4個の隣接はすべて内側5×5に収まる。利得はCpS+11%と世代あたりqueenbeet6本分(6時間CpS)の植え直し削減。以後の植付・世代同期は内側21本が対象
...
7. **shriekbulbのduketater経路(P16c)**: JQB育成中にshriekbulbが未確保なら、⑤穴(queenbeet M×5、0.1%)ではなく**duketater×3・樹齢不問(0.5%)**で狙う。JQBから遠い側の端の行にduketater×6を植え、隣の行を抽選ポケットとして空ける(中央4マスが各duke3本に接し、0.5%×4=2%/tick)。JQB湧き前は⑧穴の維持が優先のため発動しない。shriekbulb確保で自動撤収
8. **グリッドの引退**: JQBが育成中かつduketater確保済みになったら、残りのqueenbeetの植え直しを停止する(リングのelderwortは他の全JQB穴とマスを共有するため第2のJQBは湧けず、⑤穴の産出物はゴミ、shriekbulbはP16cが担うため)。残存株は成熟した時点で収穫して収穫益(1時間分CpS)を回収し、**空いたマスと穴にはbakerWheatを植える**(成熟でCpS+1%/本。最終的にJQB+リング+P16c以外の全マスが小麦になる)
...
- JQB格子は27マス植付(旧版の「26マス」は計算誤り: 36−変異穴8−(5,5)=27)。duketater/shriekbulb確保後は縁刈込で内側21本になる(§7 P16手順3)。
