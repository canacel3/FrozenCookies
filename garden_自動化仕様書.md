...
- 解禁後即: cronerice @ (0,4),(2,4),(4,4) に3本植えて放置育成(74tick≒肥料3.7h)。**以後P10の全目標が「解禁または芽あり」になるまでこの3本は撤去禁止**
- 完了: cronerice解禁または芽あり(解禁後に3本植付)

**P3: brownMold / crumbspore(雑草)**
- 条件: x=4〜5列は予約(他フェーズ・フィラーの植付を禁止し、湧き場を最低6マス保証)。**土は本フェーズ完了まで肥料固定**(ウッドチップは雑草-90%)
- 動作: meddleweedは「植付予定マスでない」かつ「隣接8マスに雑草以外の植物・植付予定マスがない」安全マスなら**盤面のどこでも飼育**し、樹齢84以上で`G.harvest`(ドロップ率=樹齢×0.1%、次tickで寿命死のため84が上限目安)。条件を満たさない雑草(親・三本柱・育成中の芽に隣接など)は汚染5%/tick対策で即撤去
- bakeberryフィラーが芽を出して引退すると空きマスが増え、湧き可能マス(隣接植物ゼロの空きマス)が約3〜6倍に拡大する
