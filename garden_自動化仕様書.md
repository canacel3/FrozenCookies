...
- **土制御**: P3完了まで肥料固定。**JQB育成中も肥料固定**(肥料3分tickはウッドチップ5分tickより実時間で約1.7倍速く熟すため)。それ以外は「アクティブなレシピの親が全員成熟→ウッドチップ / それ以外→肥料」(補助フェーズ(aux)の親は成熟判定から除外)。切替は`Date.now() >= G.nextSoil`のときのみ(10分CD)。
