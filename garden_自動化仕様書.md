...
- **短命親の植え直し抑制**: 変異は両親が同時に成熟している必要があるため、フェーズ内の別種の親(植付予定含む)の「成熟までの残りtick」を平均老化速度で見積もり、残り20tick以上かつ「相方成熟時の自身の樹齢が70超」になる植付は保留する(例: cronerice育成中のP6 thumbcornは寿命14tickで4〜5世代無駄になるため、成熟の約9tick前まで植えない)。常駐枠(三本柱・棚)の植付は対象外。
